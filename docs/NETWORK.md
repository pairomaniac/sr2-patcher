...
starting box*) puts a box on the room and presents it first. At the start line (`0x41c8d0`): state 0x81, the host sends the start
...
| chat, names in the team room, the WIN RATIO figures | the exe's own messages (`0x1b`, `0x2a`) - carried, not interpreted, except that a game message whose second byte is past the player table is dropped, as is one of the six types that carry the sender's index there (`0x1b`, `0x23`, `0x24`, `0x28`, `0x2a`, `0x2e`) naming another: the exe indexes its tables by that byte unchecked (the clock request `0x20` carries a 0 there, so it is not held to it); `GetName` answers from the roster |
...
`test.segaonline.net` instead, for trying a change.
