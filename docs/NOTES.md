...
| **The mix** | `MUSASHI\MGSound.dll` | `0x439f`, `0x6980` and appended `.sr2b` section | the sound manager - in the exe and, as a copy of the same code, in every screen DLL - gives each effect a −40..0 dB range and sets its ceiling at `(step+1)/10` of it: 4 dB a step, 0 dB at 9. The buffer's `SetRange` (`0x10004380`) loads min and max through asm/mix.asm, which makes each `7/8 × old − 800`: −43..−8, 3.5 dB a step, 9 the old 7, for every client. The streamed music - menus, settings screen, replays; every client ends in the streaming buffer's `SetVolume` (`0x10006940`) with the step × 1111 as a 0..10000 value mapped across the stream's own range - finishes that mapping through the second routine, which puts the step on the same curve plus `OFFSET` (300), 0 off |
