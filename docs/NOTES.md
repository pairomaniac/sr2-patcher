...
| **Starting box** (`starting`) | `SEGA RALLY 2.exe` | `0x367c8`, `0x359bf`, `0x35358`, `0x35002`, the annex | the two calls into the race setup (`0x438dc0`) → asm/starting.asm, which draws a box saying the race is starting on the team room's background, blits it onto the room's last frame and presents, then runs the setup; the room's draw registered as the stub's second entry, which keeps the box up; the init's surface load through its third, which takes it down; needs `lobby`. See *The starting box* |
...
The exe versions in order: 2.0.0.2 Australian and HCJ-0145,
...
a stock `SR2.CFG`'s block there at patch time, once, and `write_settings`
then writes `SR2.CFG` with the sections the applied patches read - the
controls for `xinput`, `[Display]` for `resolution`, `[Network]` for
`netplay`, as the pad annex lays them out - whole when there is none or
only the block, a missing section appended to text. `MGameReg`'s Open at
...
START, on the host, and the host's word (`0x2d`) on a guest, call the
race setup (`0x438dc0`, from `0x4373c8` and `0x4365bf`): a routine that
spins on `timeGetTime` - up to 15 s for every racer's state 0xa, a
guest's 0.5 s + 0.2 s × index stagger, the clock sync with a request a
second for up to 15 s, state 0x10 and its wait - calling only the
receive loop (`0x438720`) between looks, so no frame is drawn and the
...
`ReleaseDC` `+0x2c`) and draws the box in the middle of it as the
game's own popups look - a white border, a fill of near black (never
black: the wrapper's blit keys black out when a surface has a key),
`STARTING THE RACE` over `WAITING FOR THE OTHER PLAYERS` in white,
centred in the lobby's font (`[0x4e84c4]`), the box sized from the
lines' extents and centred on `[0x4eaea0]`'s width and height - notes
the rectangle and raises a flag, blits that rectangle alone from the
background to the back buffer through the wrapper (`+0x34` the target,
`+0x1c` the blit; under `widescreen2d` the Blt hook lands it on the
lobby surface, over the room's last frame with its names and chat, and
the present stretches it), calls MGameD3D's present and the call the
gate makes after it (`+0x80`, `+0x88` on `[0x50b118]`), then jumps to
the setup, which returns to the site. A guest's setup blocks until the
race, so that frame is what it sees; the host's returns while the
guests load, to a room that draws on (the fade to the race), and the
background's draw blits the chrome and the panels over the box. So the
room's draw, which the init registers by pushing its address
(`0x435f57`), is the stub's second entry: the exe's draw (`0x436310`),
then, while the flag is up, the box's rectangle blitted again over the
panels. The third entry stands in for the init's surface load
(`0x435c02`, `0x406fa0`): the flag down, since the background comes
back fresh without the box, then the load. gdi32 is resolved once
through the import slots. Without gdi32, a surface or a DC the first
entry goes straight to the setup. `tools/startingtest.py` runs the
three under Unicorn on every build with the surface, the present, the
draw, the load and gdi32 stubbed.
...
Resolution line and the `[Network]` section (the netplay DLL's Staging
and Log, 0 or 1) carried over as the file had them, and only when it
had them.
...
(`annex_tables`); `tools/uctest.py`'s `annex_records` and `annex_text` (the patcher's `settings_text`) model the output for the tests.
