...
| **The mix** | `MUSASHI\MGSound.dll` | `0x439f`, `0x41dc`, `0x6980` and appended `.sr2b` section | the sound manager - in the exe and, as a copy of the same code, in every screen DLL - gives each effect a −40..0 dB range, sets its ceiling at `(step+1)/10` of it from the slider (4 dB a step, 0 dB at 9) and its level as a percentage of that; the later builds pass 100, the Australian's copies pass the slider again, and fade and duck. Three routines in asm/mix.asm: the buffer's `SetRange` (`0x10004380`) loads min and max through the first, each `7/8 × old − 800` - −43..−8, 3.5 dB a step, 9 the old 7; the buffer's `SetVolume` (`0x100041c0`) loads its value through the second, any value but 0 becoming 10000, the ceiling; the streaming buffer's `SetVolume` (`0x10006940`), every client's way of setting the streamed music with the step × 1111 as its value, finishes its mapping through the third, the step on the same curve plus `OFFSET` (200), 0 off |
