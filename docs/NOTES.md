...
| **Music from files** | `MUSASHI\MGAudio.dll` | appended `.sr2m` section, 12 sites, the entry point, the CD-volume method `0x1db0` (`0x1d90` Australian) | every `call [__imp__mciSendCommandA]` → `call hook; nop`; the `mov esi, [__imp__mciSendCommandA]` at `0x10003108` → `call hookaddr; nop`; entry → the setup thunk; the volume method's entry → `jmp setvolume`, which turns the slider's 0..10000 into `waveOutSetVolume` on device 0, applied from the position polls until it takes, since mciwave opens the device on play; see [asm/README.md](../asm/README.md) |
