...
| **Mix range** | `SEGA RALLY 2.exe` | `0x72c50`, `0x72c5d`, `0x6e940` | the sound manager sets its dB range at `0x473830`: `min` −4000 stored and pushed, `max` from a getter that works out 0. A slider step is `(step+1)/10` of the range, 4 dB, to 0 dB at 9. `min` → −4300 in both places and the getter → `mov eax, -800; ret`: 3.5 dB a step, −39.5 to −8 dB, the top of the old 7. `MIX_MIN`, `MIX_MAX` in the patcher; the music curves in `asm/` are derived from them |
| **Stream level** | `MUSASHI\MGSound.dll` | `0x6980` and appended `.sr2b` section | every path that sets a stream's level - the exe's and each screen DLL's copy of the same client code - ends in the streaming buffer's `SetVolume` (`0x10006940`), the slider step × 1111 as a 0..10000 value mapped across the stream's own range (−2000..0 by default, and the client resets the ceiling from the slider too). The mapping's last step → `call` asm/bgmvol.asm, which puts the step on the effects' curve, −3950 + 350 × step, plus `OFFSET` (300), 0 off |
| **Effects at full** (Australian only) | `SEGA RALLY 2.exe` | `0xb26cb`, `0xb272e`, `0xb2782` | the volume routine sets each effect's ceiling from its slider and then its level as a percentage of that; the other builds pass 100, the Australian passes the slider × 11 - the slider twice. The setting's load → `mov eax, 9`, which the × 100 × 0.111 after it makes 100 |
| **Music from files** | `MUSASHI\MGAudio.dll` | appended `.sr2m` section, 12 sites, the entry point, the CD-volume methods `0x1db0` and `0x1e40` (`0x1d90`, `0x1e20` Australian) | every `call [__imp__mciSendCommandA]` → `call hook; nop`; the `mov esi, [__imp__mciSendCommandA]` at `0x10003108` → `call hookaddr; nop`; entry → the setup thunk; the CD-volume methods' entries → `jmp setvolume` / `jmp getvolume`: the slider's 0..10000 becomes a `waveOutSetVolume` amplitude on the effects' dB curve plus 7 dB, capped at 0, from a ten-entry table, applied after each play once the stream exists; see [asm/README.md](../asm/README.md) |
