...
| `0x435fe0` | the team room's init, from `0x436038` its status strip on DIRECT IP: `gethostbyname` and `IP Address : %d.%d.%d.%d` into the frame's buffer at `+0x20`, `TextOutA` at (150, 456) from `0x43611c`; `0x4eac0c` the network object. Patched by lobby (status) |
| `0x420f10` | the lobby entry's init (surface, x, y, field, width shown): the field copied to `0x4d3d1c`, its length to `0x4d454c`; `0x41fe20` its character handler, `0x41fef1` and `0x420849` its 0x800 cap, `0x420337` and `0x420c4c` CTRL+V from the clipboard with `lstrcpyA` at `0x4203ba` and `0x420cd5`. Patched by lobby (entrycap) |
...
| starting | 4 + section | exe `0x4373c8`, `0x4365bf` (files `0x367c8`, `0x359bf`; American `0x36af8`, `0x35cef`; Australian `0x601e8`, `0x5f3df`): the two `call 0x438dc0`, the race setup, from the host's START and the guest's on the host's word; `0x428835` (file `0x27c35`, 7 bytes; American `0x27ef5`; Australian `0x4c877` and `0x4c8ff`, its gate's two paths): the frame gate's `push eax; call [ecx+0x80]`, the present, through the annex's second entry; `0x406fa0` (file `0x63a0`, 8 bytes; American `0x407000`, Australian `0x4109f0`): the lobby's surface loader's first eight bytes, a `jmp` to the third; the annex |
...
| lobby | 19 + 3 sections + art | exe `0x43cb4e` (file `0x3bf4e`, the annex's address check), `0x420f10`, `0x41fef1`, `0x420849`, `0x4203ba`, `0x420cd5` (files `0x20310`, `0x1f2f1`, `0x1fc49`, `0x1f7ba`, `0x200d5`, the entry caps and the paste), `0x43604b` (file `0x3544b`, the status line), `0x4350e4` (file `0x344e4`, the chat line's block), `0x43bd58`, `0x43bd76` (50 bytes), `0x43bda8`, `0x43bdcc`, `0x43bddd`, `0x43bf57`, `0x43bf77`, `0x43bf85`, `0x43bfc2`, `0x4400dd`, `0x43efe0` (files `0x3b158`, `0x3b176`, `0x3b1a8`, `0x3b1cc`, `0x3b1dd`, `0x3b357`, `0x3b377`, `0x3b385`, `0x3b3c2`, `0x3f4dd`, `0x3e3e0`; the other builds' anchors in `BUILDS`); `BINDATA\connect\PROTOCOL\CONNECT.BMP` repainted, `CONNECT_{IPX,TCPIP,MODEM}_{OFF,ON,ON2}.BMP` and `button\showteam_*.BMP` rewritten, `IP_ENTRY\Ip_entry_US.bmp` redrawn under the box |
