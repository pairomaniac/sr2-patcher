#!/usr/bin/env python3
"""Run the IP entry popup's address check under Unicorn.
...
popup's sound call with the cancel sound's four arguments pushed. Needs python3-unicorn; exits 77 with a
...
from unicorn.x86_const import (UC_X86_REG_ESP, UC_X86_REG_EIP, UC_X86_REG_EBX, UC_X86_REG_ESI, UC_X86_REG_EDI,
...
def main(argv):
    ...
    image = patcher.apply_ipcheck(image, build)
    ...
    print('ipchecktest: %s: %d addresses accepted, %d entries refused with the cancel sound'
          % (build, len(ACCEPTED), len(REFUSED)))
    return 0
