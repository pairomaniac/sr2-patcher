...
| **Wave BGM turned down** | The menu loops, the settings-menu music and the replay music aren't CD tracks, and at the same slider setting the game plays them louder than the effects and the announcer. They play 6 dB lower now - about half as loud - following the BGM slider as before; the number is `ATTEN` in `asm/bgmvol.asm`. |
