...
DEVICES_BLOB = bytes.fromhex(
    'e918000000e965000000e8000000005b81eb0f00000081ebd1d1d1d1c35357e8'
    'e6ffffff8dbbd1d1d1d1c787d301000000002044c787db0100000000000083bf'
    'cf010000007523c787cf010000010000008d83d8d8d8d8508d83d9d9d9d9508d'
    '83d3d3d3d3ffd083c408ff46085f5b535755e893ffffff8dabd1d1d1d18dbbda'
    'dadada8b0785c00f849d00000083f80274486a006a006a00ff7720ff771cff77'
    '18ff7714680000803f680000803f6a006a006a00ff7710ff770c50d94708d885'
    'd3010000d91c24ff77048d83d4d4d4d4ffd083c44083c728eba9ff77108d83dc'
    'dcdcdc50ff7720ff771cff7718ff7714680000803f680000803f680000204168'
    '00002041ff770c50d94708d885d3010000d91c24ff77048d83dbdbdbdbffd083'
    'c43483c728e959ffffff83bddb0100000075328b85d301000085c0744fd985d3'
    '010000d8a5d7010000d99dd30100008b85d301000085c0796bc785d301000000'
    '000000eb5fd985d3010000d8a5d7010000d99dd301000081bdd3010000000020'
    'c47241c7460801000000eb388b83d6d6d6d68b480885c9742b8b116a01ff5214'
    'a80274208b8bd7d7d7d76a006a006a006a0e8d83d5d5d5d5ffd0c785db010000'
    '010000008d83d2d2d2d25d5f5bffe000000000000000000000204200000000'
)
MUSIC_MAGICS = {
...
# header band and hint bar are the stock sprites themselves, found by
# their first quad; text is the stock routine over its 14-px glyph
# sprites, which lack a colon, so that is a piece. Colours are (alpha,
...
PAGE_GROUP = 'PLAYER 1'
PAGE_HINT = 'PRESS B OR ESC TO GO BACK'
...
def devices_page(buf, build, va, quad_tail, cont, labelend):
    ...
    def copy_quads(sprite, n):
        """The first n quads of a stock sprite as (uv, rect, colour), its UV entries into this page."""
        off = _rva_to_off(buf, sprite - base)
        page, quads = struct.unpack_from('<II', buf, off)
        out = []
        for k in range(n):
    ...
    def add_sprite(sp, x, y, z, colour):
        draw.append((1, ('sprite', sp), x, y, z, colour))

    def add_stock(sp, x, y, z, colour):
        draw.append((1, ('stock', sp), x, y, z, colour))

    def add_text(text, x, y, flags, colour):
        strings.append(text)
        draw.append((2, ('string', len(strings) - 1), x, y, flags, colour))

    hint = copy_quads(stock['hint'], 5)
    sprites.append((hint, 432.0, 28.0))
    hint_sprite = len(sprites) - 1
    ...
    add_stock(stock['group'], PAGE_GROUP_X, PAGE_ROW_Y, Z_PLATE, PLATE)
    ...
    for i, (action, binding) in enumerate(PAGE_ROWS):
        ...
        add_stock(stock['row'], PAGE_ROW_X, y, Z_PLATE, PLATE)
    ...
    add_stock(back, 320.0, 404.0, Z_PLATE, TEXT)
    add_sprite(hint_sprite, 320.0, 451.0, Z_PLATE, TEXT)
    add_text(PAGE_HINT, 320.0, 433.0, FLAGS_PROPORTIONAL | FLAGS_CENTRE, BLACK)
    ...
    for i, (quads, w, h) in enumerate(sprites):
        ...
        for k, (x0, y0, x1, y1), colour in quads:
            ...
            q += 0x34
    for i, (kind, (what, ref), x, y, zf, colour) in enumerate(draw):
        ...
        struct.pack_into('<IIffIIIIII', blob, off_draw + i * 40, kind, ptr, x, y, z, *colour, 0)
